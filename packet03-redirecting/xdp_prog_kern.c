...
int xdp_icmp_echo_func(struct xdp_md *ctx)
{
    ...
    // old_csum = icmphdr_old.cksum;
    // icmphdr->cksum = bpf_csum_diff((__be32 *)&icmphdr_old, sizeof(icmphdr_old), (__be32 *)icmphdr, sizeof(*icmphdr), old_csum);
...
int xdp_redirect_func(struct xdp_md *ctx)
{
    ...
    int eth_type;
    int action = XDP_PASS;
    unsigned char dst[ETH_ALEN] = {0xba, 0xb5, 0x8c, 0xda, 0xa8, 0x09}; /* Assignment 2: fill in with the MAC address of the left inner interface */
...
int xdp_router_func(struct xdp_md *ctx)
{
    ...
    case BPF_FIB_LKUP_RET_UNREACHABLE: /* dest is unreachable; ca be dropped */
    case BPF_FIB_LKUP_RET_PROHIBIT:    /* dest not allowed; can ben dropped */
        action = XDP_DROP;
        break;
