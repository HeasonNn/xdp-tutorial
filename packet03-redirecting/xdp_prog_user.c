...
static int parse_mac(char *str, unsigned char mac[ETH_ALEN])
{
    ...
    int values[6];
    if (6 == sscanf(str, "%02x:%02x:%02x:%02x:%02x:%02x",
...
int main(int argc, char **argv)
{
    ...
    len = snprintf(map_filename, PATH_MAX, "%s/%s", pin_dir, "tx_port");
    if (len < 0)
    {
        fprintf(stderr, "ERR: creating map_filename\n");
        return EXIT_FAIL_OPTION;
    }
    ...
    if (map_fd < 0)
    {
        fprintf(stderr, "ERR: bpf_obj_get\n");
    }
    ...
    if (redirect_map)
    {
        ...
        len = snprintf(map_filename, PATH_MAX, "%s/%s", pin_dir, "redirect_params");
        if (len < 0)
        {
            fprintf(stderr, "ERR: creating map_filename\n");
            return EXIT_FAIL_OPTION;
