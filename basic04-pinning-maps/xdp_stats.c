...
static int stats_poll(const char *pin_dir, int map_fd, __u32 map_type, __u32 current_id, int interval)
{
...
int main(int argc, char **argv)
{
	...
	for (;;)
	{
		...
		err = stats_poll(pin_dir, stats_map_fd, info.type, info.id, interval);
		close(stats_map_fd);
		if (err < 0)
			return err;
	}
